...
/** 缓存的 Access Token 及其过期时刻（epoch 毫秒）。 */
...
export class QqBotClient {
  ...
  async accessToken(signal?: AbortSignal): Promise<string> {
    ...
    try {
      ...
      if (isAbortError(error)) throw new QqBotError('QQ access token request aborted', 'aborted', { cause: error })
    ...
    try {
      const payload = await response.json() as QqBotAccessTokenResponse
      this.cached = {
  ...
  async sendMessage(
    ...
    signal?: AbortSignal,
    msgId?: string,
    format: QqBotMessageFormat = 'text',
    ...
    try {
      ...
      if (isAbortError(error)) throw new QqBotError('QQ message send aborted', 'aborted', { cause: error })
  ...
  async gatewayUrl(signal?: AbortSignal): Promise<string> {
    ...
    try {
      ...
      if (isAbortError(error)) throw new QqBotError('QQ gateway request aborted', 'aborted', { cause: error })
    ...
    try {
      const payload = await response.json() as QqBotGatewayResponse
      return payload.url
    } catch (error: unknown) {
      if (isAbortError(error)) throw new QqBotError('QQ gateway request aborted', 'aborted', { cause: error })
...
async function readErrorMessage(response: Response, operation: string): Promise<string> {
  const status = response.status
  try {
    const payload = await response.json() as QqBotErrorResponse
    const detail = payload.message
    if (detail !== undefined && detail.length > 0) return `QQ ${operation} error: ${detail}`
  } catch (error: unknown) {
    if (isAbortError(error)) return `QQ ${operation} aborted`
